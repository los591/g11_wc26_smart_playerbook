...
@st.cache_resource
def _ph_client():
    key = st.secrets.get("POSTHOG_API_KEY", "")
    if not key or "REPLACE" in key:
        return None
    return Posthog(key, host="https://us.i.posthog.com")
...
def go_group(g):
    track("page_viewed", view="group", group=g)
...
def go_country(c):
    track("country_viewed", country=COUNTRY_DISPLAY.get(c, c), group=COUNTRY_GROUP.get(c, "?"))
    st.session_state.view = "country"
...
def go_standings():
    track("page_viewed", view="standings")
    st.session_state.view = "standings"
    st.session_state.selected_group = st.session_state.selected_country = None
    st.session_state.selected_player = 0

def go_calendar():
    track("page_viewed", view="calendar")
    st.session_state.view = "calendar"
    st.session_state.selected_group = st.session_state.selected_country = None
    st.session_state.selected_player = 0

def go_compare():
    track("page_viewed", view="compare")
...
def render_calendar():
    ...
    with tz_col:
        ...
        if selected_tz_label != saved_tz:
            track("calendar_timezone_changed", timezone=selected_tz_label)
        ...
        if q.strip():
            ...
            for r in hits:
                ...
                if st.button(
                    ...
                    track("compare_player_selected", slot=slot, player=r["player"],
                          country=r_cdisp, position=r["position"])
